...
public class FakeDatabase implements IDatabase {
	...
	/*
	 * public FakeDatabase() { authorList = new ArrayList<Author>(); bookList = new
	 * ArrayList<Book>();
	 * 
	 * // Add initial data readInitialData();
	 * 
	...
	/*
	 * public void readInitialData() { try {
	 ...
	 * title) { List<Pair<Author, Book>> result = new
	 * ArrayList<Pair<Author,Book>>(); for (Book book : bookList) { //
	 * System.out.println("Book: <" + book.getTitle() + ">" + "  Title: <" + title +
	 * ">");
	 * 
	 * if (book.getTitle().equals(title)) { Author author =
	 ...
	 * findAuthorAndBookByAuthorLastName(String lastName) { // create list of
	 * <Author, Book> for returning result of query List<Pair<Author, Book>> result
	 * = new ArrayList<Pair<Author, Book>>();
	 * 
	 * // search through table of Books for (Book book : bookList) { for (Author
	 * author : authorList) { if (book.getAuthorId() == author.getAuthorId()) { if
	 * (author.getLastname().equals(lastName)) { // if this book is by the specified
	 * author, add it to the result list result.add(new Pair<Author, Book>(author,
	 * book)); } } } } return result; }
	 */
	 ...
	 * List<Pair<Author, Book>> result = new ArrayList<Pair<Author,Book>>(); for
	 * (Book book : bookList) { Author author =
	 * findAuthorByAuthorId(book.getAuthorId()); result.add(new Pair<Author,
	 * Book>(author, book)); } return result; }
	 */
	...
	/*
	 * @Override public List<Author> findAllAuthors() { List<Author> result = new
	 ...
	 * for (Author author : authorList) { if (author.getLastname().equals(lastName)
	 * && author.getFirstname().equals(firstName)) { authorId =
	 * author.getAuthorId(); } }
	 * 
	 * // if the Author wasn't found in Authors list, we have to add new Author to
	 * Authors list if (authorId < 0) { // set author_id to size of Authors list + 1
	 * (before adding Author) authorId = authorList.size() + 1;
	 * 
	 * // add new Author to Authors list Author newAuthor = new Author();
	 * newAuthor.setAuthorId(authorId); newAuthor.setLastname(lastName);
	 * newAuthor.setFirstname(firstName); authorList.add(newAuthor);
	 * 
	 * System.out.println("New author (ID: " + authorId + ") " +
	 * "added to Authors table: <" + lastName + ", " + firstName + ">"); }
	 * 
	 * // set book_id to size of Books list + 1 (before adding Book) bookId =
	 * bookList.size() + 1;
	 * 
	 * // add new Book to Books list Book newBook = new Book();
	 * newBook.setBookId(bookId); newBook.setAuthorId(authorId);
	 * newBook.setTitle(title); newBook.setIsbn(isbn);
	 * newBook.setPublished(published); bookList.add(newBook);
	 * 
	...
	/*
	 * private Author findAuthorByAuthorId(int authorId) { for (Author author :
	 * authorList) { if (author.getAuthorId() == authorId) { return author; } }
	 * return null; }
	 */
}
